...
### 5차 사람 판정 결과 (2026-07-16, 11 qid — en 중복 tailored_blazer 병합)

| | 단일 (전) | 멀티 (후) |
|---|---|---|
| **S율** | 54.5% (6/11) | **81.8% (9/11)** |
| 비실패 (S+M) | 90.9% | **100%** |
| 실패(F) | 1 (데이트룩) | **0** |

항목별: 개선 5 (여름휴가·첫출근·데이트룩 F→S·운동·장마), 악화 2, 유지 4.

- **최대 개선**: 데이트룩 F→S — 단일 슬립드레스만으론 실패였는데 카디건+힐
  코디로 살아남. 상황 쿼리 = 코디 문제라는 가설 입증.
- **악화 2건**(우아한 미디드레스 S→M, 단정한 니트 S→M): 단일 아이템으로도
  이미 상황에 충분한 케이스는 멀티 확장이 오히려 산만하게 만듦.
  → 튜닝 포인트: "단일 대표 아이템으로 답이 되는 상황"은 확장 억제 (LLM
  프롬프트에서 sub_queries 를 더 보수적으로, 또는 대표 아이템 비중 상향).

판정 원본: `out/judgments_p9_{single_off,multi_on}.json`.
