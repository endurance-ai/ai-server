...
- **구조화 로그 이모지 범례**: 📥 webhook, 🧭 router, 👁 vision, 🔍 search, 🤔 critique/evaluator, 🎨 pipeline, 🐱 bot 발화 (respond/adapter). **트레이싱(logging-only, SPEC-AGENT-V3-REACT observability)**: 🤖 topology 배너(startup), ▶️/✅/⏭️ graph node enter/done/skip, 🔄 ReAct agent-iter, 🔧 tool dispatch, 🏁 agent 종료(respond), 🧠 v3:memory(Gap1), 🔬 v3:reflexion(Gap2 — 🤔 와 충돌 회피용 별도 마커), 💡 v3:proactive(Gap3), 🚫 v3:dislike(Gap4).
