...
async def warm_cache() -> None:
    ...
    filt: dict[str, list[str]] = {
        key: groups[next(iter(gks))] for key, gks in alias_to_groups.items() if len(gks) == 1
    }
    # Acronym fallback — unambiguous AND not already an exact alias.
    acro_idx: dict[str, list[str]] = {
        key: groups[next(iter(gks))]
        for key, gks in acro_to_groups.items()
        if len(gks) == 1 and key not in filt
    }
