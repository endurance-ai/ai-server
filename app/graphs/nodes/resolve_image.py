...
async def resolve_image(state: WorkingState) -> dict:
    ...
    if msg.photo_file_id and not msg.urls:
        try:
            adapter = get_adapter()
            image_bytes = await adapter.download_attachment(msg.photo_file_id)
        except Exception as exc:  # noqa: BLE001
            logger.exception("[resolve_image] direct upload download failed")
            breadcrumbs.append(f"resolve_image_error: download_attachment: {type(exc).__name__}"[:200])
            node_skip("resolve_image", "direct upload download failed")
        ...
        try:
            ...
            sess.image_url = data_url
            get_store().update(sess)
        except Exception:
            logger.exception("[resolve_image] session persist failed (direct upload)")
        breadcrumbs.append(f"resolve_image: direct upload ok bytes={len(image_bytes)}")
        node_done("resolve_image", resolved="direct_upload")
        return {"image_url": data_url, "log_events": breadcrumbs, "turn_no": 2}
