...
async def _load_sections(
    ...
    async with pool.connection() as conn, conn.cursor() as cur:
        ...
        for section_id, slot_type, _title, _subtitle, product_ids, _display_type in section_rows:
            # Automatic slots are independent. Only editorial sections compete
            # with one another, so an auto candidate cannot empty a themed row.
            ...
            if slot_type != "auto" or user_id is None or not (taste_scores or feature_scores):
                selected = [
                    int(pid) for pid in (product_ids or []) if not is_editorial or int(pid) not in editorial_section_ids
                ]
            else:
                await cur.execute(
                    """
                ...
                selected = select_candidate_ids(
                    candidate_rows,
                    section_id=section_id,
                    excluded_ids=editorial_section_ids if is_editorial else set(),
                    taste_scores=taste_scores,
                    feature_scores=feature_scores,
                ...
                if not selected:
                    selected = [
                        ...
                        if not is_editorial or int(pid) not in editorial_section_ids
                    ]
            selected = selected[:CURATION_SECTION_PRODUCT_LIMIT]
            selected_by_section[section_id] = selected
            if is_editorial:
                editorial_section_ids.update(selected)
