...
**핵심 의미**: 카톡은 우리 봇 인스턴스를 그대로 띄우는 게 아니라, **카카오 인프라(채널 + 오픈빌더)에 우리 스킬 서버를 webhook으로 꽂는** 형태. → portal/ai에 `/skills/kakao/*` 엔드포인트만 추가하면 검색·LLM·DB 핵심 로직은 그대로 재사용.
...
→ portal/ai에 **비동기 작업 큐 + 콜백 POST** 추가 구현 필요. LangGraph 워크플로우는 그대로 재사용.
...
5. 시나리오/블록 정의 + 스킬 등록 (portal/ai webhook URL)
