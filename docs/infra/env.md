...
`trending-search`(요즘 뜨는 브랜드)는 검색량 상승세와 최근 7일의 순 사용자 조회·저장·외부 링크 클릭, 리뷰 수를 하나의 점수로 합친 뒤 브랜드별 상위 3개 상품 점수로 브랜드 순위를 계산한다. 기존 `popular`(지금 인기) 구좌는 삭제한다. 일일 auto 구좌는 최대 300개 후보를 브랜드별 라운드로빈으로 섞어 100개를 선정한다. 브랜드 수가 적어도 브랜드별 다음 상품을 반복해서 뽑아 가능한 한 100개를 채운다. `wiki.status`는 위키 데이터 품질 상태일 뿐 큐레이션 순위나 노출 자격에 사용하지 않는다. auto와 개인화 구좌 상품은 다른 구좌에도 함께 표시한다. editorial 구좌끼리만 먼저 나온 상품을 제외한다. 브랜드 기반 editorial 구좌는 브랜드당 상한 없이 적격 상품을 라운드로빈으로 최대 200개 적재하지만, `GET /v1/curation`은 응답 크기를 제한하기 위해 모든 구좌를 앞 100개까지만 반환한다. 최근 본 상품을 제외한 각 구좌의 반환 순서는 요청마다 무작위로 섞는다.
