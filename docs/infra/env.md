...
`trending-search`(요즘 뜨는 브랜드)는 검색량 상승세와 최근 7일의 순 사용자 조회·저장·외부 링크 클릭, 리뷰 수를 하나의 점수로 합친 뒤 브랜드별 상위 3개 상품 점수로 브랜드 순위를 계산한다. 기존 `popular`(지금 인기) 구좌는 삭제한다. 일일 auto 구좌는 150개 후보에서 브랜드당 최대 2개씩 30개를 선정한다. `wiki.status`는 위키 데이터 품질 상태일 뿐 큐레이션 순위나 노출 자격에 사용하지 않는다. 일일 auto 구좌는 `trending-search` → `under-100` 순서로 선정하며, 앞 구좌에서 선택한 상품은 뒤 구좌에서 제외한다. 로그인 사용자의 요청별 개인화 재선정에도 같은 중복 제외를 적용한다. 브랜드 기반 editorial 구좌는 브랜드당 상한 없이 적격 상품을 라운드로빈으로 최대 200개 적재한다. `GET /v1/curation`은 선정된 상품의 구성은 유지하되 각 구좌 내 응답 순서를 요청마다 무작위로 섞는다.

로그인 사용자에게는 조건을 충족할 때만 `recently-viewed`(최신 조회순), `saved-on-sale`(할인 중인 찜), `taste-picks`(양수 취향 신호와 매칭), `favorite-brands`(관심 브랜드 상품) 구좌를 요청 시 계산해 추가한다. 개인화 구좌는 고정 개수 상한이 없으며 적격 상품을 모두 반환한다. 최근 본 상품은 최신순을 유지하고 나머지 구좌는 요청마다 순서를 섞는다.
