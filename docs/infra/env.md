...
`popular`은 최근 7일의 순 사용자 조회·저장·외부 링크 클릭과 리뷰 수로 상품 점수를 만든 뒤, 브랜드별 상위 3개 상품 점수를 합산해 브랜드 순위를 계산한다. auto 구좌는 150개 후보에서 브랜드당 최대 2개씩 30개를 선정한다. `wiki.status`는 위키 데이터 품질 상태일 뿐 큐레이션 순위나 노출 자격에 사용하지 않는다. auto 구좌는 `popular` → `trending-search` → `under-100` 순서로 선정하며, 앞 구좌에서 선택한 상품은 뒤 구좌에서 제외한다. 로그인 사용자의 요청별 개인화 재선정에도 같은 중복 제외를 적용한다. 브랜드 기반 editorial 구좌는 브랜드당 상한 없이 적격 상품을 라운드로빈으로 최대 200개 적재한다. `GET /v1/curation`은 선정된 상품의 구성은 유지하되 각 구좌 내 응답 순서를 요청마다 무작위로 섞는다.
