...
- 2026-05-15 (v0.3.0): **Cross-SPEC amendment driven by SPEC-AGENT-V2-REACT (REQ-AGENT-LOG-EVENT-001).** 이벤트 카탈로그에 **20번째 이벤트 타입 `tool_call`** 추가. SPEC-AGENT-V2-REACT 의 ReAct 루프가 도구를 dispatch 할 때마다 (성공/실패 무관) `ai.log_conversation_event` 에 1 row 를 fire-and-forget 으로 기록 — future ML 데이터셋(tool call trace + outcome)의 source. Payload schema: `{tool_name: str, args: dict, result_summary: dict, latency_ms: int, iteration_no: int (0-based within thread), error: str|None}`. `app/observability/event_payloads.py` 에 `ToolCallPayload` TypedDict export 추가. REQ-LOG-PAYLOAD-CAP-001 truncation 정책을 `tool_call.args` / `result_summary` 에 재귀 적용 (string 2048 / list 50 / dict 100), `error` 는 500 chars cap (`node_error.message` 와 일관). 기존 19개 이벤트 타입 처리/테스트 무변경 — 본 amendment 는 카탈로그 +1 (19→20) 및 TypedDict export 만. SPEC-AGENT-V2-REACT 의 implementation PR 은 본 v0.3.0 가 머지된 후에만 머지 가능 (REQ-AGENT-LOG-EVENT-001 BLOCKING). 본 round 는 spec.md HISTORY + frontmatter version/updated + `event_payloads.py` 변경 — SPEC-AGENT-V2-REACT 외 다른 SPEC 무변경.
